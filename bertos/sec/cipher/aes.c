...
static void AES_expandKey(BlockCipher *c_, const void *key)
{
...
static void AES_encrypt(BlockCipher *c_, void *block)
{
	...
	AddRoundKey ((uint32_t *)block, (uint32_t *)c->expkey);
	...
	for( round = 1; round < Nr + 1; round++ ) {
		...
		AddRoundKey ((uint32_t *)block, (uint32_t *)c->expkey + round * Nb);
	}
}
...
static void AES_decrypt(BlockCipher *c_, void *block)
{
	...
	AddRoundKey ((uint32_t *)block, (uint32_t *)c->expkey + Nr * Nb);
	InvShiftRows(block);
...
void AES128_init(AES128_Context *aes)
{
...
void AES192_init(AES192_Context *aes)
{
	aes->c.set_key = AES_expandKey;
	aes->c.enc_block = AES_encrypt;
	aes->c.dec_block = AES_decrypt;
...
void AES256_init(AES256_Context *aes)
{
	aes->c.set_key = AES_expandKey;
	aes->c.enc_block = AES_encrypt;
	aes->c.dec_block = AES_decrypt;
