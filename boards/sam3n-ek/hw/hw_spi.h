 ...
 * \{
 */
#define CS_ENABLE()      do { SPI0_MR = (SPI0_MR & ~SPI_PCS) | SPI_PCS_3; } while (0)
#define CS_DISABLE()     do { SPI0_MR = (SPI0_MR & ~SPI_PCS) | SPI_PCS_2; } while (0)
...
#define SPI_HW_INIT() do { \
	/* Disable PIO on SPI CS pin */ \
	PIOA_PDR = BV(22); \
	/* Peripheral B for chip select 2/3 for display and flash */ \
	PIOA_ABCDSR1 |= BV(22); \
	PIOA_ABCDSR2 &= ~BV(22); \
} while (0)
